...
const movieStore = new MovieStore({api: '', movies: JSON.stringify({response: false, results: []})});
...
class MovieSearchForm extends HTMLElement {
  ...
  connectedCallback() {
    movieStore.subscribe((state) => {
      this.setAttribute('api', state.api);
    });

    this.apiKey.addEventListener('input', (e) => {
      movieStore.apiChangedTo(e.target.value);
    });
  ...
  attributeChangedCallback(property, oldValue, newValue) {
    ...
    if (property === 'api' && this.apiKey) {
      this.apiKey.value = newValue;
    }
  }
}
